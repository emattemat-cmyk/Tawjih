...
def local_weighted_ranking(level, sec_type, subjects):
    ...
    for name,w in weights_map.items():
        ...
        ranking.append({"name":name,"score":sc})
    ranking=sorted(ranking,key=lambda x:(x["score"] is None,-(x["score"] or 0)))
    # نسبة الملائمة
    max_score=max([r['score'] for r in ranking if r['score'] is not None] or [1])
    for r in ranking:
        r['match_percent']=round((r['score'] or 0)/max_score*100,2)
    return ranking
...
def analyze():
    ...
    subjects_list=["الرياضيات","الفيزياء","العلوم الطبيعية","اللغة العربية","الفرنسية","الإنجليزية","الفلسفة"]
    subjects={}
    ...
    return jsonify({"best":best,"all":ranking,"message":"أتمنى لك التوفيق والنجاح"})
